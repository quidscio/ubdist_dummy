...
_get_vmImg_beforeBoot_ubDistBuild_sequence() {
	...
	if [[ "$3" == "" ]] # || [[ "$FORCE_AXEL" != "" ]]
	then
		_wget_githubRelease_join-stdout "soaringDistributions/ubdist_dummy" "$releaseLabel" "package_image_beforeBoot.tar.flx" | _get_extract_ubDistBuild-tar xv --overwrite
		currentExitStatus="$?"
	else
		#_wget_githubRelease_join-stdout "soaringDistributions/ubdist_dummy" "$releaseLabel" "package_image_beforeBoot.tar.flx" | _get_extract_ubDistBuild-tar --extract vm.img --to-stdout | _dd of="$3" bs=1M
		_wget_githubRelease_join-stdout "soaringDistributions/ubdist_dummy" "$releaseLabel" "package_image_beforeBoot.tar.flx" | _get_extract_ubDistBuild-tar --extract ./vm.img --to-stdout | sudo -n dd of="$3" bs=1M status=progress
		currentExitStatus="$?"
	fi
	if [[ "$currentExitStatus" != "0" ]]
	then
	...
	if [[ "$currentHash" == "" ]] || [[ "$currentHash_bytes" == "" ]]
	then
		currentHash=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_dummy" "$releaseLabel" "_hash-ubdist_beforeBoot.txt" | head -n 3 | tail -n 1)
		currentHash_bytes=$(_wget_githubRelease-stdout "soaringDistributions/ubdist_dummy" "$releaseLabel" "_hash-ubdist_beforeBoot.txt" | head -n 2 | tail -n 1 | sed 's/^.*count=$(bc <<< '"'"'//' | cut -f1 -d\  )
	fi
	( [[ "$currentHash" == "" ]] || [[ "$currentHash_bytes" == "" ]] ) && _messageFAIL
	export MANDATORY_HASH=
	unset MANDATORY_HASH
...
_get_vmImg_ubDistBuild-live_sequence() {
	...
	then
		...
		( _wget_githubRelease_join-stdout "soaringDistributions/ubdist_dummy" "$releaseLabel" "vm-live.iso" | tee >(openssl dgst -whirlpool -binary | xxd -p -c 256 >> "$scriptLocal"/hash-download.txt) ; dd if=/dev/zero bs=2048 count=$(bc <<< '1000000000000 / 2048' ) ) | sudo -n dd of="$3" bs=1M status=progress
		currentExitStatus="$?"
	fi
